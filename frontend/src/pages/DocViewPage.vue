  ...
  <div v-show="false">
      <div id="pdf-template" class="pdf-container">
        ...
        <div class="pdf-page flex flex-center column relative-position">
          ...
          <h1 class="text-weight-bolder text-center" style="color: #7d0400; font-size: 3rem; line-height: 1.2;">
            {{ doc?.title }}
          </h1>
        ...
        <div class="pdf-page q-pa-xl">
          <h3 class="text-weight-bold" style="color: #7d0400; border-bottom: 2px solid #7d0400; padding-bottom: 10px;">Índice</h3>
          
          <ul class="q-mt-lg text-h6 text-grey-9" style="line-height: 2; list-style-type: none; padding: 0;">
            <li 
            ...
            >
              <span 
                style="color: #7d0400; text-decoration: none; border-bottom: 1px dotted #ccc; transition: color 0.3s;"
              >
                {{ item.text }}
              </span>
            </li>
        ...
        <div class="pdf-page flex flex-center column" style="background-color: #7d0400; color: white;">
          <img src="/logo-cfcsn-white.png" style="max-width: 250px; filter: brightness(0) invert(1);" />
          <div class="q-mt-xl text-h5 text-weight-light">
            TI | CFCSN
          </div>
          <div class="q-mt-md text-subtitle1">
            <a :href="doc?.authorId?.linkedin" target="_blank" style="color: white; text-decoration: underline;">
...
const generatePDF = () => {
  ...
  setTimeout(() => {
    ...
    worker.toPdf().get('pdf').then((pdf: JsPdfInstance) => {
  ...
  try {
    ...
    try {
      ...
      const docProgress = myProgress.find((p: UserProgress) => {
        ...
        return targetId === doc.value?._id;
      });
      
      if (docProgress && docProgress.percentage >= 1) {
...
.markdown-body h2 { border-bottom: 1px solid #eee; padding-bottom: 0.3em; }


.markdown-body p { margin-bottom: 1.2em; }

.markdown-body a { color: var(--q-primary); text-decoration: none; }
